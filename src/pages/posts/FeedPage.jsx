...
import { followUser, getFollowing } from '@/api/friends';
...
const POST_TABS = [
  { id: 'all', label: 'All' }, { id: 'dept', label: '📢 Dept' }, { id: 'sports', label: '⚽ Sports' },
  { id: 'projects', label: '🔧 Projects' }, { id: 'qna', label: '📝 Q&A' }, { id: 'trade', label: '🛒 Trade' }, { id: 'urgent', label: '🔥 Urgent' },
];
...
export const FeedPage = () => {
  ...
  const fetchPosts = useCallback(async (pageNum = 1, currentTab = tab) => {
    ...
    try { const res = await getFeed(currentTab, pageNum); const np = res.data || res; if (pageNum === 1) setPosts(np); else setPosts(prev => [...prev, ...np]); setHasMore(res.pagination?.hasNext || false); setPage(pageNum); }
  ...
  useEffect(() => { if (!postsEnabled) return; fetchPosts(1, tab); fetchStories(); fetchSuggestions(); }, [tab, postsEnabled, fetchPosts, fetchStories, fetchSuggestions]);
  ...
  return (
    <div className="pb-20">
      ...
      {suggestions.length > 0 && (
        <div className="mb-3">
          <div className="flex items-center justify-between mb-2"><h3 className="font-bold text-[var(--color-text)] text-sm">👥 People You May Know</h3></div>
          <div className="flex gap-3 overflow-x-auto scrollbar-hide">
            {suggestions.map(person => (
              <button key={person._id} onClick={() => navigate(`/profile/${person._id}`)} className="flex flex-col items-center gap-1 flex-shrink-0 w-20">
                <Avatar src={person.avatar} name={person.firstName} size="lg" verified={person.hdmVerified} />
                <span className="text-[10px] font-semibold text-[var(--color-text)] text-center truncate w-full flex items-center justify-center gap-0.5">{person.firstName}{person.hdmVerified && <VerifiedBadge size={10} />}</span>
      ...
      {loading ? (
        <div className="space-y-3">{[...Array(3)].map((_, i) => (<div key={i} className="card p-4 space-y-3"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full skeleton" /><div className="space-y-1.5"><div className="w-24 h-3 skeleton rounded" /><div className="w-16 h-2 skeleton rounded" /></div></div><div className="w-full h-4 skeleton rounded" /><div className="w-3/4 h-4 skeleton rounded" /></div>))}</div>
      ) : posts.length === 0 ? (
        ...
        <div className="space-y-3">
          ...
          {hasMore && (<div className="text-center py-4"><Button variant="outline" size="sm" onClick={handleLoadMore} disabled={loadingMore}>{loadingMore ? 'Loading...' : 'Load More'}</Button></div>)}
        </div>
      )}
