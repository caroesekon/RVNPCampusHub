...
export const PostCard = ({ post, onDelete }) => {
  ...
  const handleShare = (e) => {
    e.stopPropagation();
    if (navigator.share) {
      navigator.share({ title: 'RVNP Post', text: post.content, url: `${window.location.origin}/post/${post._id}` });
    } else {
  ...
  const handleArchive = async (e) => {
    e.stopPropagation();
    try {
      await likePost(post._id); // Use a different endpoint for archive if available
      ...
      <div className="flex items-center gap-3 p-4 pb-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-[var(--color-text)] text-sm truncate">{post.author?.firstName} {post.author?.lastName}</span>
      ...
      {post.images?.length > 0 && (
        <div className={`px-4 pb-3 grid gap-1 ${post.images.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
          {post.images.map((img, i) => (
            <img key={i} src={img} alt="Post" className="w-full rounded-lg object-cover max-h-80" />
          ))}
        </div>
      )}
