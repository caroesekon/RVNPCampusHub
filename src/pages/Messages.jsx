      ...
      <div className="w-full">
        ...
        {aiStatus?.enabled && aiStatus?.chatEnabled && (
          <button
            ...
            className="w-full flex items-center gap-3 p-3 mb-2 rounded-xl border border-rvnp-green bg-rvnp-green bg-opacity-5 hover:bg-rvnp-green hover:bg-opacity-10 transition-all text-left cursor-pointer"
          >
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Start a conversation with friends or people you follow!"
          />
        ) : (
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${
                    isUnread
                      ? 'bg-rvnp-green bg-opacity-5 border-rvnp-green'
                      : 'bg-bg-primary border-border-color hover:bg-bg-secondary'
                  }`}
                >
                  <button
                  ...
                  >
                    ...
                    {isUnread && (
                      <span className="absolute top-0 right-0 w-3 h-3 rounded-full bg-rvnp-green border-2 border-bg-primary" />
                    )}
                  </button>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-2 mt-0.5">
                      <p className={`text-sm truncate ${isUnread ? 'font-medium text-text-primary' : 'text-text-muted'}`}>
                        {lastMessage}
                      </p>
                      {conv.unreadCount > 0 && (
                        <span className="min-w-[20px] h-5 px-1.5 rounded-full bg-rvnp-green text-rvnp-white text-xs font-semibold flex items-center justify-center shrink-0">
        ...
        <div className="space-y-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-lg bg-bg-secondary text-text-primary border border-border-color focus:outline-none placeholder:text-text-muted"
            />
          </div>
          ...
          {loadingUsers ? (
            ...
            <p className="text-center text-text-muted py-8">No one to message yet.</p>
          ) : (
            <div className="space-y-1 max-h-64 overflow-y-auto">
              {filteredUsers.map((user) => (
                <button
                  ...
                  disabled={creating}
                  className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-bg-secondary transition-all text-left disabled:opacity-50"
                >
