...
export const GroupDetailPage = () => {
  ...
  useEffect(() => { if (!group) return; if (tab === 'wall') fetchWall(); else if (tab === 'events') fetchEvents(); else if (tab === 'files') fetchFiles(); }, [tab, group]);
  ...
  const handleJoin = async () => {
    try { if (group.requiresApproval) { await requestToJoin(id); toast.success('Request sent!'); } else { await joinGroup(id); toast.success('Joined!'); } fetchGroup(); }
    catch (e) { toast.error(e.message || 'Failed'); }
  };
  const handleLeave = async () => { try { await leaveGroup(id); toast.success('Left'); fetchGroup(); } catch { toast.error('Failed'); } };
  const handlePost = async () => { if (!postContent.trim()) return; setPosting(true); try { await createPost({ content: postContent.trim(), group: id, type: 'post' }); setPostContent(''); toast.success('Posted!'); fetchWall(); } catch { toast.error('Failed'); } finally { setPosting(false); } };
  ...
  const handleSaveSettings = async () => { try { await updateGroupSettings(id, settingsForm); toast.success('Saved'); setShowSettings(false); fetchGroup(); } catch { toast.error('Failed'); } };
  ...
  return (
    <div className="pb-20">
      ...
      <div className="h-32 bg-gradient-to-r from-[var(--color-primary)]/30 to-[var(--color-primary)]/10 rounded-xl flex items-center justify-center mb-4"><span className="text-5xl">👥</span></div>
      ...
      <div className="flex items-start justify-between">
        ...
        {canManage && <button onClick={() => setShowSettings(true)} className="text-lg">⚙️</button>}
      </div>

      <p className="text-sm text-[var(--color-text-secondary)] mt-1">{group.description}</p>
      <div className="flex items-center gap-3 mt-2 text-xs text-[var(--color-text-muted)]">
        <span>👥 {formatCompactNumber(group.memberCount)} members</span><span>• {group.category}</span>
      </div>

      {group.rules?.length > 0 && (
        <button onClick={() => setShowRules(!showRules)} className="text-xs text-[var(--color-primary)] mt-2 font-semibold">📋 {group.rules.length} rule{group.rules.length > 1 ? 's' : ''} {showRules ? '▲' : '▼'}</button>
      )}
      {showRules && (
        <div className="mt-2 p-3 bg-[var(--color-bg)] rounded-lg"><ol className="list-decimal pl-4 text-xs text-[var(--color-text-secondary)] space-y-1">{group.rules.map((rule, i) => <li key={i}>{rule}</li>)}</ol></div>
      )}
      ...
      <div className="mt-3 flex gap-2 flex-wrap">
        {isMember ? (
          <>
            ...
            {canManage && <Button variant="ghost" size="sm" onClick={() => { fetchRequests(); setShowRequests(true); }}>Requests {group.joinRequests?.length > 0 ? `(${group.joinRequests.length})` : ''}</Button>}
          </>
        ) : hasRequested ? (
          ...
          {wall.length === 0 ? <EmptyState icon="📰" title="No posts yet" /> : wall.map(post => (
            <div key={post._id} className="card p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-semibold text-[var(--color-text)] text-sm flex items-center gap-1">
      ...
      {tab === 'events' && (
        <div className="space-y-3">
          {canManage && (
            <Button size="sm" variant="outline" onClick={() => { const t = prompt('Event title:'); const d = prompt('Date (YYYY-MM-DD):'); const l = prompt('Location:'); if (t && d) { createGroupEvent(id, { title: t, date: d, location: l }).then(() => fetchEvents()); } }}>+ Add Event</Button>
          )}
          {events.length === 0 ? <EmptyState icon="📅" title="No events" /> : events.map(event => (
            <div key={event._id} className="card p-4">
              ...
              {isMember && <Button size="sm" variant="outline" className="mt-2" onClick={() => handleRSVP(event._id)}>{event.going?.includes(user?._id) ? '✅ Going' : 'RSVP'}</Button>}
            </div>
          ))}
        </div>
      )}
        ...
        <div className="space-y-2">
          {files.length === 0 ? <EmptyState icon="📁" title="No files" /> : files.map(file => (
            <a key={file._id} href={file.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 rounded-lg hover:bg-[var(--color-bg)] transition-colors">
              <span className="text-2xl">📄</span><div><p className="text-sm font-semibold text-[var(--color-text)]">{file.name}</p><p className="text-xs text-[var(--color-text-muted)]">Uploaded by {file.uploadedBy?.firstName || 'member'}</p></div>
            </a>
          ))}
        </div>
      )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-[var(--color-text)] flex items-center gap-1">
                      ...
                      {m.hdmVerified && <VerifiedBadge size={10} />}
                      {isMemberAdmin && '👑'} {isMemberMod && '🛡️'}
                    </p>
                  </div>
                </div>
                {!isMemberAdmin && (
                  <div className="flex gap-1">
                    {!isMemberMod ? <button onClick={() => handleMakeMod(mid)} className="text-xs text-[var(--color-primary)] px-2 py-1 hover:bg-[var(--color-surface-hover)] rounded">Make Mod</button> : <button onClick={() => handleRemoveMod(mid)} className="text-xs text-yellow-500 px-2 py-1 hover:bg-[var(--color-surface-hover)] rounded">Remove Mod</button>}
      ...
      <Modal isOpen={showSettings} onClose={() => setShowSettings(false)} title="Group Settings">
        <div className="space-y-4">
          <Toggle enabled={settingsForm.requiresApproval} onChange={v => setSettingsForm(prev => ({ ...prev, requiresApproval: v }))} label="Require Approval" description="Members must be approved before joining" />
          <Toggle enabled={settingsForm.isPrivate} onChange={v => setSettingsForm(prev => ({ ...prev, isPrivate: v }))} label="Private Group" description="Only members can see content" />
          <div><label className="block text-sm font-semibold mb-1 text-[var(--color-text-secondary)]">Description</label><textarea value={settingsForm.description} onChange={e => setSettingsForm(prev => ({ ...prev, description: e.target.value }))} rows={2} className="input resize-none" /></div>
          <div>
            <div className="flex items-center justify-between mb-2"><label className="text-sm font-semibold text-[var(--color-text-secondary)]">Rules</label><button onClick={handleAddRule} className="text-xs text-[var(--color-primary)] font-semibold">+ Add</button></div>
            {settingsForm.rules?.map((rule, i) => (
              <div key={i} className="flex gap-2 mb-1"><input value={rule} onChange={e => handleRuleChange(i, e.target.value)} placeholder={`Rule ${i + 1}`} className="input flex-1" /><button onClick={() => handleRemoveRule(i)} className="text-[var(--color-accent)] text-sm">✕</button></div>
            ))}
          </div>
      ...
      <Modal isOpen={showRequests} onClose={() => setShowRequests(false)} title="Join Requests">
        {requests.length === 0 ? <EmptyState icon="📭" title="No pending requests" /> : requests.map(r => (
          <div key={r._id} className="flex items-center justify-between p-2 rounded-lg hover:bg-[var(--color-bg)]">
            <div className="flex items-center gap-2"><Avatar src={r.avatar} name={r.firstName} size="sm" /><p className="text-sm font-semibold text-[var(--color-text)]">{r.firstName} {r.lastName}</p></div>
            <div className="flex gap-2"><Button size="sm" onClick={() => handleApprove(r._id)}>Approve</Button><Button size="sm" variant="ghost" onClick={() => handleReject(r._id)}>Reject</Button></div>
          </div>
        ))}
      </Modal>
    </div>
